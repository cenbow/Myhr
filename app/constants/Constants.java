...
public class Constants
{
    ...
    /**
     * 易瑞特推广地址 add by lbd 20160715 增加易瑞特推广地址
     */
    public static final String URL = Play.configuration.getProperty("url", ""); // 地址
    public static final String AD_KEY = Play.configuration.getProperty("ad_key", ""); // ad_key

    /**
     * 易瑞特查询接口的key值 add by lbd 20160721
     */
    public static final String EXTENSION_KEY = Play.configuration.getProperty("Extension_key", ""); // 易瑞特查询接口的key值
    /**
     * 资金托管类型(环讯: HX; 汇付:HF)
     */
    ...
    public static final String BASE_URL = Play.configuration.getProperty("test.application.baseUrl")
    ...
    public static final boolean IPS_ENABLE = Convert.strToBoolean(Play.configuration.getProperty("pay.trustFunds"),
            false); // 是否开启资金托管模式
    ...
    public static final int DEBT_SCALE = Convert.strToInt(Play.configuration.getProperty("debt_scale"), 30); // #
    ...
    public static final String PAGE_PATH_FRONT_HOME = "@front.home.HomeAction.home"; // 前端首页
...
//    public static final String 	COMMON_URL = Play.configuration.getProperty("common_url", "");
    ...
    public static boolean IS_SECOND_BID = false; // 是否有秒还标
    ...
    public static boolean IS_EXPERIENCE_GOLD = true;// 是否使用体验金

    /**
     * 投资人手续费率上限50%（借款管理费）；借款人手续费率上限100%（理财管理费）；债权转让手续费率10%；单笔交易商户收取服务费最大费率1%（
     * 最大提现手续费率）
     * 
    ...
    public static final boolean DEV_PROD = Play.configuration.getProperty("application.mode").equals("dev") ? true
            : false;
    public static final boolean CHECK_CODE = Convert.strToBoolean(Play.configuration.getProperty("check_code"), false);// 是否需要检验普通验证码
    public static final boolean CHECK_MSG_CODE = Convert.strToBoolean(Play.configuration.getProperty("check_msg_code"),
            false);// 是否需要检验短信验证码
    public static final boolean CHECK_PIC_CODE = Convert.strToBoolean(Play.configuration.getProperty("check_pic_code"),
            false);// 是否需要检验图形验证码
    ...
    public static final String[] INIT_LABLES =
    { "借款产品简介", "借款产品详情描述", "申请人条件" };
    ...
    public static final int PICTURE_SIZE = 2 * 1024 * 1024; // 图片限制大小(10M左右)
    public static final int TXT_SIZE = 2 * 1024 * 1024; // 文本限制大小(5M左右)
    ...
    public static final int XLS_SIZE = 2 * 1024 * 1024; // 表格限制大小(5M左右)
    ...
    /** 预约中(审核通过) **/
    public static final int BID_ORDER = 30;
    ...
    /**
     * 已放款，包含：BID_REPAYMENT（4.还款中）、BID_REPAYMENTS（5.已还款）、
     * BID_COMPENSATE_REPAYMENT（14.本金垫付还款中）
     */
    public static final int BID_RELEASED = 100;
    // public static final int[] BID_AUDIT_STATUS = { 0, 1, 2, 3, 4 }; // 正向状态值
    // public static final int[] BID_FAIL_STATUS = { -1, -2, -3, -4, -5 }; //
    // 负向状态值
    ...
    public static final int V_ORDER = 30;// 预约
    ...
    { " AND (user_name LIKE ? OR bid_no LIKE ? or product_name LIKE ? or title LIKE ? )", " AND bid_no LIKE ?",
            " AND title LIKE ?", " AND user_name LIKE ?", " AND email LIKE ?", " AND product_name LIKE ?" };
    ...
    { " ORDER BY id desc ",
            ...
            " ORDER BY time ", " ORDER BY amount ", " ORDER BY order_sort ", " ORDER BY loan_schedule ",
            " ORDER BY apr ", " ORDER BY product_id ", " ORDER BY mark_overdue_time ", " ORDER BY overdue_count ",
            " ORDER BY last_repay_time ", " ORDER BY mark_bad_time ", " ORDER BY audit_time ",
            " ORDER BY real_invest_expire_time " };

    /* 会员资料审核排序数组 */
    public static final String[] ITEMS_SEARCH_ORDER =
    { " ORDER BY time desc", " ORDER BY sum_count ", " ORDER BY audited_count ", " ORDER BY not_pass_count ",
            " ORDER BY auditing_count " };
     ...
     * 查询客服关键字
     */
    public static final String[] QUERY_CUSTOMER_KEYWORD_TYPE =
    { // 全部，编号，姓名，手机，邮箱
            " and (customer_num like ? or name like ? or reality_name like ? or mobile1 like ? or email like ?) ",
            " and (customer_num like ?) ", " and (reality_name like ?) ", " and (mobile1 like ?) ",
            " and (email like ?) " };
    ...
    /**
     * 查询事件关键字 0 全部, 1 ip地址, 2 操作内容, 3 管理员名字
     */
    public static final String[] QUERY_EVENT_KEYWORD =
    { " and (ip like ? or descrption like ? or supervisor_name like ?) ", " and (ip like ?) ",
            " and (descrption like ?) ", " and (supervisor_name like ?) " };
    ...
    { "register_time desc", "recharge_amount asc", "recharge_amount desc", "invest_amount asc", "invest_amount desc",
            "invest_count asc", "invest_count desc", "bid_amount asc", "bid_amount desc", "bid_count asc",
            "bid_count desc", "order_sort asc", "order_sort desc", "audit_item_count asc", "audit_item_count desc" };

    public static final String[] STATUS_TYPE =
    { "", "and bad_bid_count > 0  ", "and bad_bid_count = 0 and  overdue_bill_count >0 ",
            "and bad_bid_count = 0 and  overdue_bill_count = 0 " };

    public static final String[] LOAN_USER_ORDER =
    { "", "bid_count asc", "bid_count desc", "overdue_bill_count asc", "overdue_bill_count desc", "bad_bid_count asc",
            "bad_bid_count desc", "audit_item_count asc", "audit_item_count desc" };

    public static final String[] INVEST_USER_ORDER =
    { "", "invest_count asc", "invest_count desc", "invest_amount asc", "invest_amount desc", "order_sort asc",
            "order_sort desc", "user_amount asc", "user_amount desc" };

    public static final String[] COMPLEX_USER_ORDER =
    { "", "invest_count asc", "invest_count desc", "invest_amount asc", "invest_amount desc", "order_sort asc",
            "order_sort desc", "user_amount asc", "user_amount desc", "bid_amount asc", "bid_amount desc",
            "repayment_amount asc", "repayment_amount desc" };

    public static final String[] VIP_USER_ORDER =
    { "", "invest_count asc", "invest_count desc", "invest_amount asc", "invest_amount desc", "num asc", "num desc",
            "bid_count asc", "bid_count desc", "bid_amount asc", "bid_amount desc", "audit_item_count asc",
            "audit_item_count desc" };

    public static final String[] CPS_TYPE =
    { "and (email like ? or name like ? or recommend_user_name like ?) ", "and email like ? ", "and name like ? ",
            "and recommend_user_name like ? " };

    public static final String[] CPS_USER_ORDER =
    { "", "commission_amount asc", "commission_amount desc", "invest_amount asc", "invest_amount desc",
            "bid_amount asc", "bid_amount desc", "recharge_amount asc", "recharge_amount desc" };

    public static final String[] UNVERIFIED_USER_ORDER =
    { "", "recharge_amount desc", "recharge_amount asc" };

    public static final String[] LOCKED_USER_ORDER =
    { "", "recharge_amount asc", "recharge_amount desc", "lock_time asc", "lock_time desc", "user_amount asc",
            "user_amount desc" };

    public static final String[] REPORTED_USER_ORDER =
    { "", "reported_count asc", "reported_count desc", "user_amount asc", "user_amount desc" };

    public static final String[] BLACK_USER_ORDER =
    { "", "reported_count asc", "reported_count desc", "order_sort asc", "order_sort desc", "user_amount asc",
            "user_amount desc" };

    /**
     * 前台--cps推广
     */
    public static final String[] MY_CPS =
    { "", "and is_active = 1 ", "and is_active = 0 " };
    public static final String[] WITHDRAWAL_TYPE =
    { "", "and status = 2 ", "and status = 0 ", "and status = 1 ", "and status = -1 " };
    ...
    { "and (bid_no like ? or name like ? or bill_no like ?) ", "and bid_no like ? ", "and name like ? ",
            "and bill_no like ? " };

    public static final String[] BILL_ORDER_Maturity =
    { " id desc", "repayment_time asc", "repayment_time desc", "apr asc", "apr desc", "amount asc", "amount desc",
            "overdue_time asc", "overdue_time desc", "overdue_count asc", "overdue_count desc" };

    public static final String[] BILL_ORDER_PAID =
    { " id desc", "repayment_time", "repayment_time desc", "apr", "apr desc", "amount", "amount desc", "overdue_time",
            "overdue_time desc", "" };

    public static final String[] BILL_ORDER_OVERDUE =
    { " id desc", "repayment_time asc", "repayment_time desc", "apr asc", "apr desc", "amount asc", "amount desc",
            "overdue_time asc", "overdue_time desc", "late_penalty asc", "late_penalty desc" };

    public static final String[] BILL_ORDER_RECEIVE =
    { " real_repayment_time desc", "repayment_time asc", "repayment_time desc", "apr asc", "apr desc", "amount asc",
            "amount desc", "overdue_time asc", "overdue_time desc", "real_repayment_time asc",
            "real_repayment_time desc" };

    public static final String[] BILL_ORDER_RECEVIABLE =
    { " id desc", "bills_timely_completion_rate asc", "bills_timely_completion_rate desc", "overdue_counts asc",
            "overdue_counts desc", "uncollected_rate asc", "uncollected_rate desc" };

    public static final String[] BILL_ORDER_REPAYMENT =
    { " id desc", "receive_time asc", "receive_time desc", "pay_amount asc", "pay_amount desc", "overdue_time asc",
            "overdue_time desc", "real_receive_time asc", "real_receive_time desc" };

    public static final String[] BILL_ORDER_INVEST =
    { " id desc", "ontime_complete_rate asc", "ontime_complete_rate desc", "principal_advances_rate asc",
            "principal_advances_rate desc", "nopaid_rate asc", "nopaid_rate desc", "nopaid_amount asc",
            "nopaid_amount desc" };

    public static final String[] BILL_ORDER_STATUS =
    { "", "and status = 0 ", "and status = -2 " };
    public static final String[] INVEST_BILL_HAS_PAID =
    { " id desc", " receive_time asc ", " receive_time desc ", " pay_amount asc ", " pay_amount desc",
            " real_receive_time asc ", " real_receive_time desc" };

    /**
     * 后台--提现管理
     */
    public static final String[] WITHDRAWAL_ORDER_TYPE =
    { "", "order by time asc", "order by time desc", "order by amount asc", "order by amount desc",
            "order by audit_time asc", "order by audit_time desc", };

    /**
     * 后台--本金保障
     */
    public static final String[] SIDE =
    { "and (from_pay_name like ? or to_receive_name like ?) ", "and to_receive_name like ? ",
            "and from_pay_name like ? " };

    public static final String[] TYPE =
    { "and (detail.type = 2 or detail.type = 1) ", "and detail.type = 1 ", "and detail.type = 2 " };

    public static final String[] NEW_ORDER =
    { "order by time desc", "order by read_count", "order by time", "order by start_show_time",
            " and location_pc > 0 order by time", " and is_use = 0 order by time" };

    public static final String[] BID_APR_CONDITION =
    { " ", " and apr <= 10 ", "  and apr > 10 and apr < 15 ", " and apr >= 15 and apr <= 20  ", " and apr > 20 " };

    public static final String[] BID_AMOUNT_CONDITION =
    { " ", " and amount <= 100000 ", "  and amount > 100000 and amount <= 500000  ",
            " and amount > 500000 and amount <= 1000000 ", "  and amount > 1000000 and amount <= 3000000 ",
            "  and amount > 3000000 " };

    public static final String[] BID_LOAN_SCHEDULE_CONDITION =
    { " ", " and loan_schedule <= 50 ", " and loan_schedule > 50  and loan_schedule <=80 ",
            " and loan_schedule > 80  and loan_schedule <100 ", " and loan_schedule =100 " };

    public static final String[] BID_ORDER_CONDITION =
    { " order by loan_schedule asc", " order by amount desc ", " order by amount asc", " order by apr desc ",
            " order by apr asc", " order by loan_schedule desc ", " order by loan_schedule asc",
            " ORDER BY (CASE period_unit WHEN (- 1) THEN (360 * period) WHEN 0 THEN (30 * period) ELSE period END) DESC ",
            " ORDER BY (CASE period_unit WHEN (- 1) THEN (360 * period) WHEN 0 THEN (30 * period) ELSE period END) ASC" };

    public static final String[] DEBT_AMOUNT_CONDITION =
    { " ", "  and  debt_amount <= 1000 ", "  and  debt_amount > 1000  and debt_amount <=5000 ",
            "  and  debt_amount > 5000   and debt_amount <=10000 ",
            "  and  debt_amount > 10000 and  debt_amount <=30000 ", "  and  debt_amount > 30000 " };

    public static final String[] DEBT_ORDER_CONITION =
    { " ", " order by  debt_amount desc", " order by  debt_amount asc", " order by  apr desc", " order by  apr asc",
            " order by  end_time desc", " order by  end_time asc", " order by  repayment_time desc",
            "order by  repayment_time asc" };

    public static final String[] TRANSFER_MANAGEMENT_STATUS_CONDITION =
    { " ", " and status = 0 ", " (and status = 1  or status = 2)", " and status = 3 ",
            " (and status = -2  or  status = -3  or status = -5)   ", " and status = -1 " };

    public static final String[] TRANSFER_MANAGEMENT_TYPE_CONDITION =
    { "   ", "  and title like ? ", "  and bid_no like ? " };

    /**
     * 前台--我的借款账单
     */
    public static final String[] LOAN_BILL_REPAYMENT =
    { "", "and status in (-1, -2) ", "and status in (-3, 0) " };
    public static final String[] LOAN_BILL_OVDUE =
    { "", "and is_overdue in (0) ", "and is_overdue in (-1, -2, -3) " };
    public static final String[] LOAN_BILL_ALL =
    { "and title like ? ", "and title like ? " };

    /**
     * 前台--我的理财账单
     */
    public static final String[] LOAN_INVESTBILL_RECEIVE =
    { "", "and status in (-1, -2,-5,-6) ", "and status in (-3 ,-4, 0) " };
    public static final String[] LOAN_INVESTBILL_OVDUE =
    { "", "and status in (-1, 0,-5) ", "and status in (-4, -2, -3,-6) " };
    public static final String[] LOAN_INVESTBILL_ALL =
    { "and title like ? ", "and title like ? " };
    ...
    { " order by time desc", " order by time desc", " order by appended_items_count desc",
            " order by appended_items_count", " order by passed_items_count desc", " order by passed_items_count",
            " order by unpassed_items_count desc", " order by unpassed_items_count",
            " order by auditing_items_count desc", " order by auditing_items_count" };

    /**
     * 理财情况统计表
     */
    public static final String[] INVEST_STATISTICS =
    { "", "order by invest_fee_back desc ", "order by invest_fee_back ", "order by average_invest_amount desc ",
            "order by average_invest_amount ", "order by invest_count desc ", "order by invest_count " };

    /**
     * 审核科目库统计分析表-排序类型
     */
    public static final String[] AUDIT_ITEMS_ORDER_TYPE =
    { " order by id", " order by id desc", " order by risk_control_ranking desc", " order by risk_control_ranking ",
            " order by submit_per desc", " order by submit_per ", " order by relate_overdue_bid_num desc",
            " order by relate_overdue_bid_num ", " order by relate_bad_bid_num desc", " order by relate_bad_bid_num " };

    /**
     * 审核科目库统计分析表-搜索关键字
     */
    public static final String[] AUDIT_ITEMS_CONDITION =
    { " and (no like ? or name like ?)", " and (no like ?)", " and (name like ?)" };

    /**
     * 借款情况统计分析表-排序类型
     */
    public static final String[] BORROW_ORDER_TYPE =
    { " order by id desc", " order by id desc", " order by month desc", " order by month ",
            " order by total_borrow_amount desc", " order by total_borrow_amount ", " order by overdue_per desc",
            " order by overdue_per ", " order by bad_bill_amount_per desc", " order by bad_bill_amount_per " };

    /**
     * 借款标销售情况分析表-排序类型
     */
    public static final String[] PRODUCT_ORDER_TYPE =
    { " order by id desc", " order by id desc", " order by released_bids_num desc", " order by released_bids_num ",
            " order by average_bid_amount desc", " order by average_bid_amount ", " order by success_bids_num desc",
    ...
    { " order by id desc", " order by id asc", " order by id desc", " order by recharge_amount asc",
            " order by recharge_amount desc", " order by recharge_menber asc", " order by recharge_menber desc",
            " order by new_recharge_menber asc", " order by new_recharge_menber desc", };

    /**
     * 保障本金统计分析表-排序类型
     */
    public static final String[] SECURITY_ORDER_TYPE =
    { " order by id desc", " order by id asc", " order by id desc", " order by bad_debt_income_rate asc",
            " order by bad_debt_income_rate desc", " order by pay asc", " order by pay desc",
            " order by advance_acount asc", " order by advance_acount desc", };

    /**
     * 会员数据统计分析表-排序类型
     */
    public static final String[] MEMBER_ORDER_TYPE =
    { " order by id desc", " order by id asc", " order by id desc", " order by new_member asc",
            " order by new_member desc", " order by new_recharge_member asc", " order by new_recharge_member desc", };

    public static final String[] IPS_TYPE =
    { "01", "128", "04", "16", "32", "1024" };
    public static final String[] GO_TYPE =
    { "ICBC", "CMB", "CCB", "ABC", "BOC", "SPDB", "CIB", "BOBJ", "CEB", "BOCOM", "CMBC", "CITIC", "GDB", "PAB", "PSBC",
            "SRCB", "BOS", "HXBC", "TCCB", "NBCB", "NJCB" };
    ...
    public static final int CPSREWARDTYPE_USER_DEALCOUNT = 2;// 推广规则：1:-按会员数
                                                             // 2-按交易额
    ...
    /** 直客码前缀 */
    public static final String NORMAL_SPREAD_PREFIX = "T"; // 理财邀请码前缀
    ...
    public static final int EXPORT_ALL = 2;// 全部导出
    /**
     * 充值来源标志
     */
    public static final String RECHARGE_APP = "2"; // app充值
    public static final String RECHARGE_WECHAT = "3"; // 微信充值
    public static final String RECHARGE_PC = "1"; // 微信充值
     ...
     * 同一个手机号码，间隔时间-单位s
     */
    public static final long MOBILE_INTERVALTIME = Long
            .parseLong(Play.configuration.getProperty("mobile_interval_time", "30"));

    /**
     * 同一个手机号码，单位时间内短信发送次数
     */
    public static final int MOBILE_TIMEEFFECTCOUNT = Integer
            .parseInt(Play.configuration.getProperty("mobile_time_effect_count", "30"));

    /**
     * 同一个IP，间隔时间-单位s
     */
    public static final long IP_INTERVALTIME = Long.parseLong(Play.configuration.getProperty("ip_interval_time", "0"));

    /**
     * 同一个IP，单位时间内短信发送次数
     */
    public static final int IP_TIMEEFFECTCOUNT = Integer
            .parseInt(Play.configuration.getProperty("ip_time_effect_count", "300"));

    /**
     * 借款意向列表
     */
    public static final String[] APPLICATION_IN_ORDER_TYPE =
    { " order by id desc", " order by amount asc", " order by amount desc", " order by time asc",
            " order by time desc", };
    /**
     * 红包统计列表
     */
    public static final String[] BONUS_ORDER =
    { "time desc", "use_time asc", "use_time desc", "amount asc", "amount desc" };

    /** 体验金状态 **/
    public static final int GOLD_EXPIRE = -1;// 已过期
    public static final int GOLD_UNUSED = 0;// 未使用
    public static final int GOLD_INHAND = 1;// 处理中
    public static final int GOLD_USED = 2;// 已使用

    /** id5接口URL **/
    public static final String ID5_URL = Play.configuration.getProperty("id5_url", "");

    /** 实名认证appkey **/
    public static final String APP_KEY = Play.configuration.getProperty("app_key", "");

    /**
     * 体验金发放统计表
     */
    public static final String[] GOLD_ORDER_TYPE =
    { " order by id desc", " order by id asc", " order by id desc", " order by experience_amount asc",
            " order by experience_amount desc", " order by invest_use_time asc", " order by invest_use_time desc",
            " order by used_amount asc", " order by used_amount desc", };

    /**
     * 易宝借记卡参数(快捷支付) 工商银行 光大银行 建设银行 农业银行 招商银行 中国银行 邮储银行 广发银行 兴业银行 浦发银行 民生银行 中信银行
     * 平安银行 深发展银行 上海银行 华夏银行
     */
    public static final String[] YB_BANK_REQ =
    { "ICBC-NET-B2C", "CEB-NET-B2C", "CCB-NET-B2C", "ABC-NET-B2C", "CMBCHINA-NET-B2C",

            "BOC-NET-B2C", "POST-NET-B2C", "GDB-NET-B2C", "CIB-NET-B2C", "SPDB-NET-B2C",

            "CMBC-NET-B2C", "ECITIC-NET-B2C", "PINGANBANK-NET-B2C", "SDB-NET-B2C", "BCCB-NET-B2C",

            "SHB-NET-B2C", "HXB-NET-B2C", "BOCO-NET-B2C",

            "ICBC-NET-B2B", "CEB-NET-B2B", "CCB-NET-B2B", "ABC-NET-B2B", "CMBCHINA-NET-B2B",

            "BOC-NET-B2B", "CMBC-NET-B2B", "BOCO-NET-B2B"

    };

    /**
     * 易宝 #商户编号 #商户密钥 #在线付款请求地址 #易宝支付请求地址 #查询和退款地址
     */
    public static final long YB_GATEWAY = 3;
    public static final String YB_P1MERID = Play.configuration.getProperty("p1_MerId", "");
    public static final String YB_KEYVALUE = Play.configuration.getProperty("keyValue", "");
    public static final String YB_ONLINEPAYMENTREQURL = Play.configuration.getProperty("onlinePaymentReqURL", "");
    public static final String YB_YEEPAYCOMMONREQURL = Play.configuration.getProperty("yeepayCommonReqURL", "");
    public static final String YB_QUERYREFUNDREQURL = Play.configuration.getProperty("queryRefundReqURL", "");
    public static final String YB_MER_URL = BASE_URL + Play.configuration.getProperty("yb_mer_url", "");
    public static final String YB_MER_BACK_URL = BASE_URL + Play.configuration.getProperty("yb_mer_back_url", "");
    public static final String YB_QueryReqURL = Play.configuration.getProperty("QueryReqURL", "");

    /**
     * 淘宝短信接口短信模板ID 短信类型(1：充值成功；2：投标成功；3：放款成功；4：成功回款；5：成功还款；6：提现申请)
     */
    public static final String RECHARGE_SMS_ID = Play.configuration.getProperty("recharge_sms_id", "");// 充值
    public static final String INVEST_SMS_ID = Play.configuration.getProperty("invest_sms_id", "");// 充值
    public static final String LOAN_SMS_ID = Play.configuration.getProperty("loan_sms_id", "");// 放款
    public static final String RECEIVE_SMS_ID = Play.configuration.getProperty("receive_sms_id", "");// 回款
    public static final String PAYMENT_SMS_ID = Play.configuration.getProperty("payment_sms_id", "");// 还款
    public static final String WITHDRAWAL_SMS_ID = Play.configuration.getProperty("withdrawal_sms_id", "");// 提现

    public static final String MOBILE_SMS_ID = Play.configuration.getProperty("mobile_sms_id", "");// 手机短信验证码

    public static final String SMS_PRODUCT_NAME = Play.configuration.getProperty("sms_product_name", "拓米金融");// 短信中平台产品名称

    /**
     * 乾璟通短信接口短信模板ID 短信类型(1：充值成功；2：投标成功；3：放款成功；4：成功回款；5：成功还款；6：提现申请)
     */
    public static final String MESSAGE_URL = Play.configuration.getProperty("message_url", "");// 短信接口地址
    public static final String ACCOUNT_NUMBER = Play.configuration.getProperty("account_number", "");// 乾璟通账号
    public static final String ACCOUNT_PASSWORD = Play.configuration.getProperty("account_password", "");// 乾璟通账号密码

    public static final String INDENTIFYING_CODE = Play.configuration.getProperty("identifying_code", "");// 验证码
    public static final String RECHARGE_MESSAGE = Play.configuration.getProperty("recharge_message", "");// 充值短信
    public static final String WITHDRAWALS_MESSAGE = Play.configuration.getProperty("withdrawals_message", "");// 提现短信
    public static final String LOAN_MESSAGE = Play.configuration.getProperty("loan_message", "");// 放款短信
    public static final String PAYMENT_MESSAGE = Play.configuration.getProperty("payment_message", "");// 回款短信
    public static final String REPAYMENT_MESSAGE = Play.configuration.getProperty("repayment_message", "");// 还款短信
    public static final String INVEST_MESSAGE = Play.configuration.getProperty("invest_message", "");// 投资短信
    public static final String REGISTERED_GIFT_EXPERIENCE = Play.configuration.getProperty("registered_gift_experience",
            "");// 注册赠送体验金
    public static final String SEND_RED_PACKAGE = Play.configuration.getProperty("send_red_package", "");// 实名注册用户送188元红包
    public static final String BID_RED_BONUS = Play.configuration.getProperty("bid_red_bonus", "");// 投标红包获得奖励
    public static final String OVERDUE_RED_ENVELOPE = Play.configuration.getProperty("overdue_red_envelope", "");// 红包过期（按天累计）
    public static final String CPS_RECOMMENDED_AWARD = Play.configuration.getProperty("cps_recommended_award", "");// CPS推荐奖励
    public static final String CPS_COMMISSION_INCENTIVE = Play.configuration.getProperty("cps_commission_incentive",
            "");// CPS佣金奖励
    /**
     * 快乐赚配置 add by lbd 20160810
     */

    public static final String KLZ_URL = Play.configuration.getProperty("klz_url", ""); // 快乐赚推广地址
    public static final String KLZ_ADID = Play.configuration.getProperty("klz_adID", ""); // 快乐赚推广adID
    public static final String KLZ_KEY = Play.configuration.getProperty("klz_key", ""); // 快乐赚推广加密key

    /**
     * 投天下配置 add by lbd 20160812
     */

    public static String SECRET = "24156051d115e944e15eab504e620b54";

    public static String BID_URL = Play.configuration.getProperty("wdtx_bid_url", "");

    public static String INVEST_URL = Play.configuration.getProperty("wdtx_invest_url", "");

    public static String PROGRESS_URL = Play.configuration.getProperty("wdtx_progress_url", "");
}
