...
public class Page  implements Serializable{
	...
	private String getBackstageThemeScript(){
		...
		//转到第几页：<span>转到<input type="text" class="l-text txt_test" value="1">页</span> 
		strBuff.append("<span>转到<input type=\"text\" id=\"goPage"+defaultMethod+"\" class=\"l-text txt_test\" value=\"" + currPage + "\">页</span>");
		
		//跳转按钮<a href="javascript:void(0)" title="" target="">GO</a> 
		strBuff.append("<a id=\"" + defaultMethod +"\" href=\"javascript:void(0)\" onclick=\"jumpPage(" + pageSize + ",this)\" title=\"\" target=\"\">GO</a>");
		
		strBuff.append("</div>");
		String script = "<script type=\"text/javascript\">  "
				+ "$(\"#" + defaultMethod + "\").click(function() {"
				+ "var pageSize = "+ pageSize + ";"
				+ "var currPage = $(this).prev('span').children('input').val().trim();"
				+ "var rule = /^\\+?[1-9][0-9]*$/;"
				+ "if(!rule.test(currPage)){"
				+ "alert(\"页码只能是正整数，请重新输入!\");"
				+ "return;"
				...
				+ "if(currPage > "+ this.totalPageCount + "){"
				+ "alert(\"输入页数大于当前最大页!\");"
				+ "$(\"#goPage"+defaultMethod+"\").val("+this.totalPageCount+");"
				+ "currPage = "+ this.totalPageCount
				+ "}"
				+ defaultMethod + "(currPage,pageSize);"
				+ "});"
				+ "</script>";
		strBuff.append(script);
		return strBuff.toString();
