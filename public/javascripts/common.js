...
function uploadImageType(fileElementId, imageElementId) {
	...
	$.ajaxFileUpload({
		...
		success : function(data) {
			...
			$("#" + imageElementId).attr("src", data1[0].filePath);
			$("#imageSize").val(data1[0].size);
			$("#imageType").val(data1[0].fileType);
			$("#imageResolution").val(data1[0].resolution);
			$("#imageType2").html(data1[0].fileType);
			$("#imageResolution2").html(data1[0].resolution);
		},
		error : function(data, status, e) {
			alert("上传图片失败");
		}
...
function replaceAllHTML(content){
	...
	content = content.replaceAll("'", "html_y");
...
//正则范围：0.0-99.9，可输入一位小数
function proportionRange(obj){
  obj.value=obj.value.replace(/[^\d\.]/g, '').replace(/^\.+/, '').replace(/^(\d{1,2}(\.\d{0,1})?).*/,'$1');
}
//正则范围：0.0-9.9或者10，可输入一位小数，10的时候不可用输入
function discountRange(obj){
	obj.value=obj.value.replace(/[^\d\.]/g, '').replace(/^\.+/, '').replace(/^(10|\d{0,1}(\.\d{0,1})?).*/,'$1');
}
//正则范围：1-12，正整数
function zeroToTwelfth(obj){
  obj.value=obj.value.replace(/[^\d]/g,'').replace(/(^12|11|10|(\d{0,1})).*/, '$1');
}
//正则范围：0.00-9999999.99,可输入2位小数，7位整数位
function withdrawRange(obj){
  obj.value=obj.value.replace(/[^\d\.]/g,'').replace(/^\.+/,'').replace(/^(\d{1,7}(\.\d{0,2})?).*/,'$1');
}
//正则范围：0.00-20位整数,可输入2位小数，
function maxRange(obj){
	obj.value=obj.value.replace(/[^\d\.]/g,'').replace(/^\.+/,'').replace(/^(\d{1,18}(\.\d{0,2})?).*/,'$1');
}
//正则范围：0.00-99.99或者100, 可输入2位小数，100的时候不可以输入小数位
function percentage(obj){
  obj.value=obj.value.replace(/[^\d\.]/g,'').replace(/^\.+/,'').replace(/^(100|\d{1,2}(\.\d{0,2})?).*/,'$1');
}
//正则范围：0到n位的最大值，正整数。n表示最大位数
function ndigitInteger(obj,n){
  var reg = new RegExp('^(\\d\{0,'+n+'\}).*');
  obj.value=obj.value.replace(/[^\d]/g,'').replace(reg,'$1');
}
...
/*songjia 我的账户左侧导航栏选中样式*/
function myAccountSelect(index1, index2) {
	$("#li_"+index1).addClass("cur")
	$("#li_"+index1).find("dl").removeClass("none");
	$("#child"+index2).addClass("cur");
};